...
impl<'a> Iterator for Tokenizer<'a> {
    ...
    //     // Minimum is 0 tokens, maximum is remaining bytes (one token per byte)
    //     let remaining = self.len.saturating_sub(self.index);
    //     (0, Some(remaining))
    // }
}

// // Add ExactSizeIterator implementation since we can calculate remaining tokens
// impl<'a> ExactSizeIterator for Tokenizer<'a> {
//     #[inline(always)]
//     fn len(&self) -> usize {
...
// // Add FusedIterator since we never return Some after None
...
mod tests {
    ...
    fn test_all_features() {
        ...
        for (&expected_tag, token) in expected_tags.iter().zip(Tokenizer::new(source)) {
